...
cat /tmp/haproxy_workers_ip | xargs -I IP scp -o StrictHostKeyChecking=no -i ~/.ssh/terraformwp.pem  /tmp/filesysid_for_mount.txt ec2-user@IP:/tmp/filesysid_for_mount.txt
while read host; do ssh -i ~/.ssh/terraformwp.pem ec2-user@$host bash -s < /tmp/mount.sh & done < /tmp/haproxy_workers_ip
